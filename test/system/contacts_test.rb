require "application_system_test_case"

class ContactsTest < ApplicationSystemTestCase
  setup do
    @contact = contacts(:one)
  end

  test "visiting the index" do
    visit contacts_url
    assert_selector "h1", text: "Contacts"
  end

  test "creating a Contact" do
    visit contacts_url
    click_on "New Contact"

    fill_in "Email home", with: @contact.email_home
    fill_in "Email work", with: @contact.email_work
    fill_in "Full name", with: @contact.full_name
    fill_in "Notes", with: @contact.notes
    fill_in "Organization", with: @contact.organization
    fill_in "Phone home", with: @contact.phone_home
    fill_in "Phone mobile", with: @contact.phone_mobile
    click_on "Create Contact"

    assert_text "Contact was successfully created"
    click_on "Back"
  end

  test "updating a Contact" do
    visit contacts_url
    click_on "Edit", match: :first

    fill_in "Email home", with: @contact.email_home
    fill_in "Email work", with: @contact.email_work
    fill_in "Full name", with: @contact.full_name
    fill_in "Notes", with: @contact.notes
    fill_in "Organization", with: @contact.organization
    fill_in "Phone home", with: @contact.phone_home
    fill_in "Phone mobile", with: @contact.phone_mobile
    click_on "Update Contact"

    assert_text "Contact was successfully updated"
    click_on "Back"
  end

  test "destroying a Contact" do
    visit contacts_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Contact was successfully destroyed"
  end
end
